 ...
 * @Author:         aijiu
 * @CreateDate:     2022/4/27 14:28
 * @UpdateUser:     aijiu
 * @UpdateDate:     2022/4/27 14:28
 * @UpdateRemark:   更新说明
 * @Version:        1.0
 */
object OSUtils {
     ...
     * ROM 类型
     */
    private val ROM_TYPE = initRomType()
        ...
        try {
            ...
            if (buildProperties.containsKey(KEY_MIUI_VERSION_NANE) || buildProperties.containsKey(
                ...
                if (buildProperties.containsKey(KEY_MIUI_VERSION_NANE)) {
                    val versionName: String = buildProperties.getProperty(KEY_MIUI_VERSION_NANE)
                    if (!TextUtils.isEmpty(versionName) && versionName.matches(Regex("[Vv]\\d+"))) { // V8
                        try {
                            rom.baseVersion = versionName.split("[Vv]").toTypedArray()[1].toInt()
                        } catch (e: Exception) {
                            e.printStackTrace()
                        }
                    }
                ...
                if (buildProperties.containsKey(KEY_MIUI_VERSION)) {
                    val versionStr: String = buildProperties.getProperty(KEY_MIUI_VERSION)
                ...
                if (buildProperties.containsKey(KEY_DISPLAY_ID)) {
                    val versionStr: String = buildProperties.getProperty(KEY_DISPLAY_ID)
                    val matcher: Matcher =
                ...
                if (buildProperties.containsKey(KEY_COLOROS_ROM_VERSION)) {
                    val versionStr: String = buildProperties.getProperty(KEY_COLOROS_ROM_VERSION)
                    val matcher: Matcher =
                ...
                if (buildProperties.containsKey(KEY_FUNTOUCHOS_OS_VERSION)) {
                    val versionStr: String = buildProperties.getProperty(KEY_FUNTOUCHOS_OS_VERSION)
                ...
                if (buildProperties.containsKey(KEY_EUI_VERSION)) {
                    val versionStr: String = buildProperties.getProperty(KEY_EUI_VERSION)
                    val matcher: Matcher =
                ...
                val displayId: String = buildProperties.getProperty(KEY_DISPLAY_ID)
                ...
                val baseOsVersion: String = buildProperties.getProperty(KEY_BASE_OS_VERSION)
                ...
                val clientIdBase: String = buildProperties.getProperty(KEY_CLIENT_ID_BASE)
                when (clientIdBase) {
